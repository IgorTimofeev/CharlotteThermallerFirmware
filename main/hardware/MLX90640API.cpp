...
{
     return MLX90640I2CDriverRead(slaveAddr, 0x2400, 832, eeData);
}
...
int MLX90640_GetSubFrameData(uint8_t slaveAddr, uint16_t *frameData)
{
    ...
    while(dataReady == 0)
    {
        error = MLX90640I2CDriverRead(slaveAddr, 0x8000, 1, &statusRegister);
        if(error != 0)
        {
            return error;
        }
    ...
    error = MLX90640I2CDriverWrite(slaveAddr, 0x8000, 0x0030);
    if(error == -1)
    {
        return error;
    }
    ...
    //reading frame
    error = MLX90640I2CDriverRead(slaveAddr, 0x0400, 832, frameData);
    if(error != 0)
    {
        return error;
    }

    error = MLX90640I2CDriverRead(slaveAddr, 0x800D, 1, &controlRegister1);
...
int MLX90640_SetResolution(uint8_t slaveAddr, uint8_t resolution)
{
    ...
    if(error == 0)
    {
        ...
        error = MLX90640I2CDriverWrite(slaveAddr, 0x800D, value);
    }
